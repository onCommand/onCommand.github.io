...
$(document).ready(() => {
    const links = [{
            name: 'جمع مواد داعش منذ 2018',
            link: '',
        },
        {
            name: 'أكثر من 200 غيغابايت من مقاطع الفيديو / الصور',
            link: '',
        },
        {
            name: 'جهة الاتصال البرقية الحقيقية الوحيدة لدي هي "@rainakhi"',
            link: '',
        },
        {
            name: 'موثوق به في داعش منذ عام 2020',
            link: '',
        },
        {
            name: 'لا اله الا الله محمد رسول الله',
            link: '',
        },
    ];
    ...
    for (let i in links) {
        ...
        $('#marquee').append(`<a href="https://t.me/rainakhi" target="_BLANK">${link.name}</a>`);
    ...
    app.titleChanger(['إِنَّا لِلَّهِ وَإِنَّا إِلَيْهِ رَاجِعُونَ', 'قُلْ هُوَ اللَّهُ أَحَدٌ', 'لا اله الا الله محمد رسول الله', 'وَقُل رَّبِّ زِدْنِي عِلْمًا', 'وَلَا تَحْزَنْ إِنَّ اللَّهَ مَعَنَا', 'فَبِأَيِّ آلَاءِ رَبِّكُمَا تُكَذِّبَانِ', 'إِنَّ مَعَ الْعُسْرِ يُسْرًا', 'وَقُل رَّبِّ زِدْنِي عِلْمًا']);
...
$.getJSON(ipgeolocation, (data) => {
    writeLine(['Authenticating...', "Granting access to <span style='font-size: 14px; color: #06d;'>[unknown]</span>..."], 30, () => {
        ...
        writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Welcome back, <i style='color: #0f0'>${ip}</i>! By the way, nice to see someone from ${country} here!`], 30, 500, () => {
            ...
            writeLine([`<i style='color: #F62459'>antithense top. $$$</i>`], 120, 500, () => {
                timeouts.push(
                    setTimeout(() => {
                        if (app.skippedIntro) return;
